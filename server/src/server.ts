...
wss.on('connection', async (ws: WebSocket) => {
  ...
  ws.on('close', async () => {
    // If the socket disconnects properly, use this statement.
    if (socketManager!.status === SOCKET_STATUS.CONNECTED) {
    ...
    socketManager!.setStatus(SOCKET_STATUS.DISCONNECTED);
    socketManager!.disconnect({ subscriber: true, publisher: true });
